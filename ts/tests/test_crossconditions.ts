...
test("crossconditions", assert => withTestProjectCopy(async controller => {
	...
	let summator = controller.addProject({
		...
		getLaunchCommand: (): string[] => {
			return [controller.nodeEnv.nodeExecutablePath, summator.imploderConfig.outFile, "Result: "]
		},
	...
	let multiplier = controller.addProject({
		...
		imploderProfile: "dev",
		getLaunchCommand: (): string[] => {
			return [controller.nodeEnv.nodeExecutablePath, multiplier.imploderConfig.outFile, "Result: "]
		},
